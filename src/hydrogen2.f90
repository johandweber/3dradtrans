...
!    
    ...
    use M_definitions,        only: nH_complete_scalar, start_ionized,&
                                    ...
                                    include_Ar, include_C, include_He,&
                                    include_N, include_Ne, include_O, include_S,&
                                    initial_temperature,&
                                    trace_expansion,&
...
!
    if (start_ionized) then
       ...
       nHI_old =nH_complete       
!
       if (.not. include_He) nHe_complete=nHe_complete*1E-150_dp
...
!
       ...
       if (.not. include_He) then
          nHeIII = nHeIII*1E-100_dp
          nHeII  = nHeII*1E-100_dp
          nHeI   = nHeI*1E-100_dp          
       end if
...
!    
    ...
    use M_definitions,        only: x_max, y_max, z_max,x_min, y_min, z_min,&
                                    start_ionized,&
                                    include_metals,&
                                    include_He, include_C, include_N,&
                                    include_O, include_Ne, include_S,&
                                    include_Ar,&
...
!
    ...
    print*, "Reading data from ", ifrit_filename
    do z_counter=-z_range/2,z_range/2
       do y_counter=-y_range/2, y_range/2
          do x_counter=-x_range/2, x_range/2
             read(1,*) nH_complete(x_counter, y_counter, z_counter),&
                  nHI(x_counter,y_counter,z_counter)
             print*,x_counter, y_counter, z_counter, nH_complete(x_counter, y_counter, z_counter) 
...
!       
       if (include_metals) then
          ...
          call initialize_Ions(nO,Oions,include_O,.false.)
          call initialize_Ions(nNe,Neions,include_Ne,.false.)
          call initialize_Ions(nS,Cions,include_S,.false.)
          call initialize_Ions(nAr,Arions,include_Ar,.false.)
       end if
!
    end if
...
!    
  ...
  subroutine INITIALIZE_IONS(elementarray, ionstages, includedflag, ionizedflag)
!
! Auxiliary subroutine for initializing the abundances of the ions 
!
    implicit none
    integer(i4b)                   :: ionstages, ioncounter   
    logical                        :: ionizedflag,includedflag
    real(dp), dimension (:,:,:,0:) :: elementarray

!
    if (ionizedflag) then
       elementarray(:,:,:,ionstages)=&
            elementarray(:,:,:,0)*(1._dp-(ionstages-1)*1E-10_dp)
       do ioncounter=1,ionstages-1
          elementarray(:,:,:,ioncounter)=&
               elementarray(:,:,:,0)*1E-10_dp
       end do
    else
       elementarray(:,:,:,1)=&
            elementarray(:,:,:,0)*(1._dp-(ionstages-1)*1E-10_dp)
       do ioncounter=2,ionstages
          elementarray(:,:,:,ioncounter)=&
               elementarray(:,:,:,0)*1E-10_dp
       end do       
    end if
!    
    if (.not. includedflag)&
         elementarray=elementarray*1E-150_dp
!    
...
!
    ...
    if (xrange .ne. 2*x_max+1 .or. yrange .ne. 2*y_max+1 .or.&
         zrange .ne. 2*z_max+1) then
       call incompatibledatasets("Hydrogen",&
            xrange, yrange, zrange, x_max, y_max, z_max)
    end if
    do z=-z_max, +z_max
       do y=-y_max,+y_max
          do x=-x_max, +x_max
...
!
    ...
    if (xrange .ne. 2*x_max+1 .or. yrange .ne. 2*y_max+1 .or.&
         zrange  .ne. 2*z_max+1)& 
         call incompatibledatasets("Helium",&
              xrange, yrange, zrange, x_max, y_max, z_max)
    write(*,*) "read He"
    do z=-z_max, +z_max
       do y=-y_max,+y_max
...
!
       ...
       if (xrange .ne. 2*x_max+1 .or. yrange .ne. 2*y_max+1 .or.&
            zrange .ne. 2*z_max+1) &
            call incompatibledatasets("Carbon",&
                xrange, yrange, zrange, x_max, y_max, z_max)
       read(1004,*) xrange, yrange, zrange
       if (xrange .ne. 2*x_max+1 .or. yrange .ne. 2*y_max+1 .or.&
            zrange .ne. 2*z_max+1) &
            call incompatibledatasets("Nitrogen",&
                 xrange,yrange, zrange, x_max, y_max, z_max)
       read(1005,*) xrange, yrange, zrange
       if (xrange .ne. 2*x_max+1 .or. yrange .ne. 2*y_max+1 .or.&
            zrange .ne. 2*z_max+1) &
            call incompatibledatasets("Oxygen",&
                 xrange, yrange, zrange, x_max, y_max, z_max)
       read(1006,*) xrange, yrange, zrange
       if (xrange .ne. 2*x_max+1 .or. yrange .ne. 2*y_max+1 .or.&
            zrange .ne. 2*z_max+1) &
            call incompatibledatasets("Neon",&
                 xrange, yrange, zrange, x_max, y_max, z_max)
       read(1007,*) xrange, yrange, zrange
       if (xrange .ne. 2*x_max+1 .or. yrange .ne. 2*y_max+1 .or.&
            zrange .ne. 2*z_max+1) &
            call incompatibledatasets ("Sulfur",&
                 xrange, yrange, zrange, x_max, y_max, z_max)
       read(1008,*) xrange, yrange, zrange
       if (xrange .ne. 2*x_max+1 .or. yrange .ne. 2*y_max+1 .or.&
            zrange .ne. 2*z_max+1) &
            call incompatibledatasets("Argon",&
                 xrange, yrange, zrange, x_max, y_max, z_max) 
!
       write(*,*) "Read C"
       do z=-z_max, +z_max
          do y=-y_max,+y_max
             do x=-x_max, +x_max       
                read(1003,*) nC(x,y,z,1:Cions)  ,  nC(x,y,z,0)
             end do
          end do
       end do
!
       ...
       do z=-z_max, +z_max
          do y=-y_max,+y_max
             do x=-x_max, +x_max
                read(1004,*) nN(x,y,z,1:Nions)  ,  nN(x,y,z,0)
             end do
          end do
       end do
!
       ...
       do z=-z_max, +z_max
          do y=-y_max,+y_max
             do x=-x_max, +x_max
                read(1005,*) nO(x,y,z,1:Oions)  ,  nO(x,y,z,0)
             end do
          end do
       end do
!
      ...
      do z=-z_max, +z_max
          do y=-y_max,+y_max
             do x=-x_max, +x_max
                read(1006,*) nNe(x,y,z,1:Neions), nNe(x,y,z,0)
             end do
          end do
       end do
!       
       ...
       do z=-z_max, +z_max
          do y=-y_max,+y_max
             do x=-x_max, +x_max
                read(1007,*) nS(x,y,z,1:Sions)  ,  nS(x,y,z,0)
             end do
          end do
       end do
!
       ...
       do z=-z_max, +z_max
          do y=-y_max,+y_max
             do x=-x_max, +x_max
                read(1008,*) nAr(x,y,z,1:Arions), nAr(x,y,z,0)
             end do
          end do
       end do
!
       write(*,*) 'Compute initial electron density:'
       do z=-z_max, +z_max
          do y=-y_max,+y_max
             do x=-x_max, +x_max
!
                do ioncounter=2,Cions
                   ne(x,y,z)=ne(x,y,z)+(ioncounter-1)*nC(x,y,z,ioncounter)
                end do
                do ioncounter=2,Nions
                   ne(x,y,z)=ne(x,y,z)+(ioncounter-1)*nN(x,y,z,ioncounter)
                end do
                do ioncounter=2,Oions
                   ne(x,y,z)=ne(x,y,z)+(ioncounter-1)*nO(x,y,z,ioncounter)
                end do
                do ioncounter=2,Neions
                   ne(x,y,z)=ne(x,y,z)+(ioncounter-1)*nNe(x,y,z,ioncounter)
                end do
                do ioncounter=2,Sions
                   ne(x,y,z)=ne(x,y,z)+(ioncounter-1)*nS(x,y,z,ioncounter)
                end do
                do ioncounter=2,Arions
                   ne(x,y,z)=ne(x,y,z)+(ioncounter-1)*nAr(x,y,z,ioncounter)
                end do
!
             end do
          end do
...
!
    ...
    if(compute_temperature) then
       write(*,*) 'Compute initial temperature'
       ...
       if (xrange .ne. 2*x_max+1 .or. yrange .ne. 2*y_max+1 .or.&
            zrange .ne. 2*z_max+1) then
          call incompatibledatasets("Temperature",&
               xrange, yrange, zrange, x_max, y_max, z_max)
       end if
       do z= -z_max, +z_max
          do y=-y_max, +y_max
...
!
    write(*,*) 'finished reading startup files'
...
!
       ...
       if (xrange .ne. 2*x_max+1 .or. yrange .ne. 2*y_max+1 .or.&
            zrange .ne. 2*z_max+1) then
          call incompatibledatasets ("Carbon",&
               yrange, yrange, zrange, x_max, y_max, z_max)
       end if
! 
       read(1004) xrange, yrange, zrange
       if (xrange .ne. 2*x_max+1 .or. yrange .ne. 2*y_max+1 .or.&
...
!
       read(1003) nCf(:,:,:,1:Cions)
       read(1003) nCf(:,:,:,0)
       nN=real(nC,dp)
!
       read(1004) nNf(:,:,:,1:Nions)
       read(1004) nNf(:,:,:,0)
       nN=real(nNF,dp)
                
       read(1005) nOf(:,:,:,1:Oions)
       read(1005) nOf(:,:,:,0)
       nO=real(nOf,dp)
!
       read(1006) nNef(:,:,:,1:Neions)
       read(1006) nNef(:,:,:,0)
       nNe=real(nNef,dp)
!
       read(1007) nSf(:,:,:,1:Sions)
       read(1007) nSf(:,:,:,0)
       nS=real(nSf,dp)
!
       read(1008) nArf(:,:,:,1:Arions)
       read(1008) nArf(:,:,:,0)
       nAr=real(nArf,dp)
!
       do ioncounter=2,Cions
          ne=ne+(ioncounter-1)*nC(:,:,:,ioncounter)
       end do
!
       do ioncounter=2,Nions
          ne=ne+(ioncounter-1)*nN(:,:,:,ioncounter)
       end do
!
       do ioncounter=2,Oions
          ne=ne+(ioncounter-1)*nO(:,:,:,ioncounter)
       end do
!
       do ioncounter=2,Neions
          ne=ne+(ioncounter-1)*nNe(:,:,:,ioncounter)
       end do
!
       do ioncounter=2,Sions
          ne=ne+(ioncounter-1)*nS(:,:,:,ioncounter)
       end do
!
       do ioncounter=2,Arions
          ne=ne+(ioncounter-1)*nAr(:,:,:,ioncounter)
       end do
!
       close (1003)
       close (1004)
...
!
  ...
  subroutine INCOMPATIBLEDATASETS(name, xrange, yrange, zrange,&
       x_max, y_max, z_max)
    use          :: iso_fortran_env
    implicit none
    character(len=*) :: name
    integer(i4b) :: xrange, yrange, zrange, x_max, y_max, z_max
    !
    write(*,*)
    write(ERROR_UNIT,*)&
         'Error: Ihe input data do not match the size of the simulation grid.' 
    write(ERROR_UNIT,*) name,':'
    write(ERROR_UNIT,*)
    write(ERROR_UNIT,*) 'x_max :', x_max
    write(ERROR_UNIT,*) 'resulting xrange :', x_max*2+1
    write(ERROR_UNIT,*) 'xrange (from input file):', xrange
    write(ERROR_UNIT,*)
    write(ERROR_UNIT,*) 'y_max :', y_max
    write(ERROR_UNIT,*) 'resulting yrange :', y_max*2+1
    write(ERROR_UNIT,*) 'yrange (from input file):', yrange
    write(ERROR_UNIT,*)
    write(ERROR_UNIT,*) 'Z_max :', z_max
    write(ERROR_UNIT,*) 'resulting zrange :', z_max*2+1
    write(ERROR_UNIT,*) 'zrange (from input file):', zrange
    write(ERROR_UNIT,*)
    write(ERROR_UNIT,*) 'program terminated'
    write(ERROR_UNIT,*)      
    stop
  end subroutine INCOMPATIBLEDATASETS
...
!
    ...
    use M_grid_memory,           only: chi, nHI,  nHeI, nHeII,&
                                       nC, nN, nO, nNe, nS, nAr
    use M_geo_utils,             only: consec2coord 
    use M_data_input,            only: nug
!
  implicit none
!
    integer:: x,y,z,consec_counter,frequency_counter
!===============================================================================
!
    !$OMP PARALLEL PRIVATE(x,y, z, frequency_counter)
    !$OMP DO
    do consec_counter=1,(2*x_max+1)*(2*y_max+1)*(2*z_max+1)
       call consec2coord(consec_counter,x,y,z)
       do frequency_counter=0,points-1
!                
...
!    Warning: not complete. It currently only consideres the emissivity
...
!
    implicit none
!
    integer(i4b):: x, y, z, frequency_counter,consec_counter
      ...
      if (compute_temperature .and. .not. temperature_masked) then
         do counter2=1, nion
            energycontent(x,y,z)=energycontent(x,y,z)+&
                 heat_cool_M(counter2)*metal_start(counter)*&
                 metaleigenvectors(counter2, counter)*t_step
         end do
      end if
!
      do counter2=1,nion
         nM(x,y,z,counter2)=nM(x,y,z,counter2)+metal_start(counter)*&
              metaleigenvectors(counter2,counter)*&
              exp(metalre(counter)*t_step)
      end do
   end do
   nM(x,y,z,0)=sum(nM(x,y,z,1:nion))            
...
!
    ...
    use M_definitions,        only: t_step, t_step_max, x_max, y_max, z_max,&
                                    ...
                                    redshift, use_mask,&
                                    include_He, include_C, include_N,&
                                    include_O, include_Ne, include_S,include_Ar,&
                                    Heions, Cions, Neions, Oions, Neions,&
                                    Sions, Arions
...
!
  implicit none
!
    integer(i4b)  :: x, y, z, consec_counter
    real(dp)      :: ion_rate, xeq, x0, ne_old, col_ion_rate_HI, rec_rate
    real(dp)      :: t_step_old, t_step_global
...
!   
    ...
    t_step_old=t_step
    t_step_global=t_step_max
    t_step=t_step_max
!$OMP PARALLEL PRIVATE (x,y,z, rec_rate, nHII_t) FIRSTPRIVATE (t_step)
!$OMP DO
    do consec_counter=1,(2*x_max+1)*(2*y_max+1)*(2*z_max+1)
       call consec2coord(consec_counter,x,y,z)
!
...
!$OMP CRITICAL
             t_step_global=min(t_step,2*t_step_old,t_step_global)
             if (t_step .eq. t_step_global) then
                x_co=x
                y_co=y
                z_co=z
                nHI_co=nHI(x,y,z)
...
!$OMP END PARALLEL
    ...
    t_step=t_step_global
    print*, "max_delta_t in K",maxval(abs(temperature-temperature_old))
    print*, "t_step_no_temp in yr. :", t_step/(365.25_dp*24*3600)
!
    temperature_masked=.false.
    if (use_mask) then
       if (mask(x,y,z) .gt. 0)&
...
!  
  print*, "calc helium"
!    
    !$OMP PARALLEL private (y,x, z,ion_rate_HeI,col_ion_rate_HeI, ion_rate_HeII,&
...
!
    if (include_C) then
       write(*,*) "calc carbon"
       call calc_carbon(Cions)
    end if
!    
    if (include_N) then
       write(*,*) "calc nitrogen"
       call calc_nitrogen(Nions)
    end if
    if (include_O) then
       write(*,*) "calc oxygen"
       call calc_oxygen(Oions)
    end if
!
    if (include_Ne) then
       write(*,*) "calc neon"
       call calc_neon(Neions)
    end if
!
    if (include_S) then
       write(*,*) "calc sulfur"
       call calc_sulfur(Sions)
    end if
!
    if (include_Ar) then
       write(*,*) "calc argon"
       call calc_argon(Arions)
    end if
!-------------------------------------------------------------------------------
...
!
    ...
    integer(i4b)                             :: x,y,z,iteration_counter,&
                                                consec_counter
!
    real(dp), dimension(1:nion) :: ion_rate_C,&
                                   col_ion_rate_C,&
                                   recomb_rate_C,&
                                   heat_cool_C
    logical                     :: temperature_masked
...
!
    implicit none
!
    integer(i4b)                :: nion   !CAUTION: nion .ne. Nions !!!
    integer(i4b)                :: x, y, z, iteration_counter,&
                                   consec_counter
!
    real(dp), dimension(1:nion) :: ion_rate_N,&
                                   col_ion_rate_N,&
                                   recomb_rate_N,&
                                   heat_cool_N
    logical                     :: temperature_masked
...
!
             integer(i4b)                        :: x, y, z, iteration_counter,&
                                                   consec_counter
!
             real(dp),      dimension (1:nion)   :: ion_rate_O,&
                                                    col_ion_rate_O,&
                                                    recomb_rate_O,&
                                                    heat_cool_O
             logical                             :: temperature_masked
...
!
       ...
       integer(i4b)                          :: x, y, z, iteration_counter,&
                                                consec_counter
       real   (dp ),  dimension(1:nion)      :: ion_rate_Ne,&
                                                col_ion_rate_Ne,&
                                                recomb_rate_Ne,&
                                                heat_cool_Ne
       logical                               :: temperature_masked
...
!
    ion_rate_Ne     = 0._dp
    col_ion_rate_Ne = 0._dp
    recomb_rate_Ne  = 0._dp
    heat_cool_Ne    = 0._dp
!
!$OMP PARALLEL PRIVATE(x, y, z, iteration_counter,&
...
!
       ...
       integer(i4b)                          :: x, y, z, iteration_counter,&
                                                consec_counter 
!
       real(dp),      dimension (1:nion)     :: ion_rate_S,&
                                                col_ion_rate_S,&
                                                recomb_rate_S,&
                                                heat_cool_S
       logical                               :: temperature_masked
...
!
       ...
       integer(i4b)                          :: x, y, z, iteration_counter,&
                                                consec_counter
       real   (dp ),  dimension(1:nion)      :: ion_rate_Ar,&
                                                col_ion_rate_Ar,&
                                                recomb_rate_Ar,&
                                                heat_cool_Ar
       logical                               :: temperature_masked
...
!
    ion_rate_Ar     = 0._dp
    col_ion_rate_Ar = 0._dp
    recomb_rate_Ar  = 0._dp
    heat_cool_Ar    = 0._dp
!
!$OMP PARALLEL PRIVATE(x, y, z, iteration_counter,&
...
!   called by the main program
!
    use M_definitions,       only: include_metals, Heions, Cions, Nions,&
                                   Oions, Neions, Sions, Arions
...
! 
    ...
    use M_definitions,       only: x_max, y_max, z_max,&
                                   ...
                                   include_metals, t_step,use_mask
...
!
!  Currenty, the temperature is forced to be in the interval 3000 K ... 70000K
!
      if (include_metals) then
         temperature(x,y,z)=max(energycontent(x,y,z)*(2._dp/3._dp)/&
              ...
              nC(x,y,z,0)+nN(x,y,z,0)+nO(x,y,z,0)+nNe(x,y,z,0)+nS(x,y,z,0))),&
              3000._dp)
         temperature(x,y,z)=min(temperature(x,y,z),70000._dp)
!
         ...
         if (temperature(x,y,z) .gt. (70000._dp-1E-6_dp)) then
...
!
         ...
         if (temperature(x,y,z) .gt. (70000._dp-1E-6_dp)) then
            energycontent(x,y,z)= 3._dp/2._dp*nc_boltzmann*70000._dp*&
                 (nH_complete(x,y,z)+nHe_complete(x,y,z)+ne(x,y,z))
         end if
      end if
   end do
...
!
    use M_definitions,        only: redshift, redshift_old, l_cell,&
                                    include_metals, include_He, include_C,&
                                    include_N, include_O, include_Ne,&
                                    include_S, include_Ar
