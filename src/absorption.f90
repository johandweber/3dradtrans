            ...
            do f=1, points-1
               ...
               do rr_counter=1,diffuse_random_rays
                  ...
                  do while (&
                     ...
                     if (x .eq. -33 .and. y .eq.-33 .and.&
                          z .eq. -33 .and. e .eq. 0) then
!!$                       print*,"absorbed_freq_diffuse:",absorbed_freq_diffuse
!!$                       print*, "photons_freq_diffuse:",  photons_freq_diffuse
                     end if
...
!
   ...
   do so=1, num_sources
      ...
      !$OMP PRIVATE (errstat,r,c,f,e,length_sum,tau, absorbed_freq, photons_freq,&
         ...
         do r=t,rays-1, num_threads
            do c=1,8
               do f=1,points-1
                  ...
                  do while (&
                       abs(source_x(so)+coord_sign(1,c)*&
                       (ray_cells(e,r)%x-x_min)) .le. x_max .and. &
                       abs(source_y(so)+coord_sign(2,c)*&
...
!                     
                     ...
                     tau(f,c)=chi( source_x(so)+coord_sign(1,c)*&
...
!                     
                     ...
                     if ((chi(&
                        source_x(so)+coord_sign(1,c)*(ray_cells(e,r)%x-x_min),&
                          source_y(so)+coord_sign(2,c)*(ray_cells(e,r)%y-y_min),&
                          source_z(so)+coord_sign(3,c)*(ray_cells(e,r)%z-z_min)&
                          ,f)) .ne. 0) then
!                       !$OMP ATOMIC  
                        j_nu(source_x(so)+coord_sign(1,c)*&
                             ...
                             (ray_cells(e,r)%z-z_min),f)+&
                             absorbed_freq(f,c)/&
                             (chi( source_x(so)+coord_sign(1,c)*&
                             (ray_cells(e,r)%x-x_min),&
                             source_y(so)+coord_sign(2,c)*&
                             (ray_cells(e,r)%y-y_min),&
                             source_z(so)+coord_sign(3,c)&
